...
async function submitOrder() { const phone = $('#orderPhone').value.trim(); const name = $('#orderName').value.trim() || 'CIA Smart Menu'; const comment = $('#orderComment').value.trim(); const items = cartItems(); if (!items.length) return; const button = $('#submitOrder'); button.disabled = true; button.textContent = 'Отправка…'; $('#orderError').hidden = true; try { const response = await fetch(`${API_BASE}/api/create-order`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ confirm: true, spot_id: 1, phone, first_name: name, comment, products: items.map(({ product, count }) => ({ product_id: Number(product.id), count })) }) }); const data = await response.json(); if (!response.ok || !data.ok) throw new Error(data.error || 'Не удалось отправить заказ'); state.cart.clear(); closeCart(); const success = $('#success'); success.hidden = false; success.textContent = 'Заказ отправлен в Poster.'; renderCart(); window.setTimeout(() => { success.hidden = true; success.textContent = ''; }, 3000); } catch (e) { $('#orderError').hidden = false; $('#orderError').textContent = e.message; } finally { button.disabled = false; button.textContent = 'Отправить заказ'; } }
