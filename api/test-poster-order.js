...
export default async function handler(req, res) {
  ...
  if (!token) return res.status(500).json({ ok: false, error: 'POSTER_TEST_ACCESS_TOKEN is not configured on the backend.' });
  ...
  try {
    ...
    if (productId !== 46) return res.status(400).json({ ok: false, error: 'This test endpoint only allows Cola product ID 46.' });
    if (!Number.isInteger(spotId) || spotId < 1) return res.status(400).json({ ok: false, error: 'Invalid spot_id.' });
    if (!Number.isInteger(count) || count < 1 || count > 20) return res.status(400).json({ ok: false, error: 'Count must be between 1 and 20.' });
    ...
    const posterResponse = await fetch(`https://joinposter.com/api/incomingOrders.createIncomingOrder?token=${encodeURIComponent(token)}`, {
      ...
      body: JSON.stringify({ spot_id: spotId, phone, first_name: firstName, last_name: lastName, comment, products: [{ product_id: productId, count }] })
    });
    ...
    if (!posterResponse.ok) return res.status(502).json({ ok: false, error: 'Poster API returned an HTTP error.', posterStatus: posterResponse.status, poster: posterData });
