...
const prepackById = new Map(
  (prepackNutrition.results || [])
    .filter((entry) => entry.status === 'calculated' && entry.per100g)
...
async function posterRequest(method, params = {}) {
  ...
  const response = await fetch(url, { headers: { Accept: 'application/json', 'User-Agent': 'CIA-Smart-Menu-Dish-Recalc/1.0' }, signal: AbortSignal.timeout(30000) });
...
function calculateRows(rows) {
  ...
  for (const row of rows) {
    ...
    if (grams == null) {
      unresolved.push({ id: String(row?.ingredient_id ?? ''), type: isPrepack ? 'prepack' : 'ingredient', name: String(row?.ingredient_name || '').trim(), reason: 'unknown_weight_or_unit' });
      continue;
    }
    ...
    if (isPrepack) {
      ...
      if (prep?.per100g) {
        values = {
          calories: n(prep.per100g.calories),
          protein: n(prep.per100g.protein),
          fat: n(prep.per100g.fat),
          carbohydrates: n(prep.per100g.carbohydrates)
        };
        if (!Object.values(values).every((v) => v != null)) values = null;
      }
    } else {
      values = nutrientValues(verifiedIngredient(String(row?.ingredient_id ?? '')));
    }

    if (!values) {
      unresolved.push({ id: String(row?.ingredient_id ?? ''), type: isPrepack ? 'prepack' : 'ingredient', name: String(row?.ingredient_name || '').trim(), grams: round1(grams), reason: isPrepack ? 'prepack_nutrition_unresolved' : 'verified_nutrition_missing' });
      continue;
    }
    ...
    knownGrams += grams;
    resolvedRows += 1;
    if (isPrepack) resolvedPrepackRows += 1;
  }
...
function makeNutrition(result) {
  ...
  if (result.fullyResolved) {
    return {
      ...
      protein: round1(result.totals.protein),
      fat: round1(result.totals.fat),
      carbohydrates: round1(result.totals.carbohydrates),
      servingGrams: round1(result.totalGrams),
      per100g: per100(result.totals, result.totalGrams),
      partial: null,
      source
    };
  }
  if (!(result.knownGrams > 0)) {
    return { status: 'needs_review', calories: null, protein: null, fat: null, carbohydrates: null, servingGrams: null, per100g: null, partial: null, source };
  }
  const gramCoverage = result.totalGrams > 0 ? result.knownGrams / result.totalGrams : 0;
  const rowCoverage = result.rowsLength > 0 ? result.resolvedRows / result.rowsLength : 0;
  const coverage = Math.max(0, Math.min(0.999, Math.min(gramCoverage || rowCoverage, rowCoverage)));
  return {
    status: 'needs_review',
    calories: null,
    protein: null,
    ...
    partial: {
      ...
      totalIngredients: result.rowsLength,
      coverage: round1(coverage * 1000) / 1000,
      per100g: per100(result.totals, result.knownGrams)
    },
    source
  };
}
...
async function main() {
  ...
  const ids = [...productById.keys()].filter(Boolean);

  const details = await mapLimit(ids, 6, async (productId) => {
    try {
      return { productId, detail: await posterRequest('menu.getProduct', { product_id: productId }) };
    } catch (error) {
  ...
  const summary = {
    ...
    productsWithPrepackRows: 0,
    resolvedPrepackRows: 0,
    unresolvedPrepackRows: 0,
  ...
  for (const item of details) {
    if (item.error) {
      ...
      summary.products.push({ productId: item.productId, status: 'fetch_error', error: item.error });
      continue;
    }
    summary.fetchedProductCount += 1;
    const rows = Array.isArray(item.detail?.ingredients) ? item.detail.ingredients : [];
    if (!rows.length) continue;
    summary.productsWithRecipe += 1;

    const result = calculateRows(rows);
    result.rowsLength = rows.length;
    if (result.prepackRows > 0) summary.productsWithPrepackRows += 1;
    summary.resolvedPrepackRows += result.resolvedPrepackRows;
    summary.unresolvedPrepackRows += result.prepackRows - result.resolvedPrepackRows;

    const nutrition = makeNutrition(result);
    if (nutrition.status === 'calculated') summary.fullyCalculatedCount += 1;
    else if (nutrition.partial) summary.partialCount += 1;
    else summary.reviewWithoutKnownCount += 1;

    const target = productById.get(item.productId);
    if (target) target.nutrition = nutrition;
    summary.products.push({
      productId: item.productId,
      name: target?.name || { ru: item.detail?.product_name || '' },
      status: nutrition.status,
      displayedCalories: nutrition.status === 'calculated' ? nutrition.calories : nutrition.partial?.calories ?? null,
      displayedAsMinimum: nutrition.status !== 'calculated' && Boolean(nutrition.partial),
      recipeRows: rows.length,
      resolvedRows: result.resolvedRows,
      prepackRows: result.prepackRows,
      resolvedPrepackRows: result.resolvedPrepackRows,
      unresolved: result.unresolved
    });
  }

  snapshot.exportedAt = new Date().toISOString();
  snapshot.source = 'firestore_public_menu_snapshot + local_live_poster_nutrition_with_prepacks';
  fs.writeFileSync(snapshotPath, JSON.stringify(snapshot, null, 2) + '\n');
  fs.writeFileSync(summaryPath, JSON.stringify(summary, null, 2) + '\n');

  console.log(`[Dish nutrition] public=${summary.publicProductCount}; fetched=${summary.fetchedProductCount}; fetchErrors=${summary.fetchErrorCount}`);
  console.log(`[Dish nutrition] recipes=${summary.productsWithRecipe}; withPrepackRows=${summary.productsWithPrepackRows}`);
  console.log(`[Dish nutrition] prepackRows resolved=${summary.resolvedPrepackRows}; unresolved=${summary.unresolvedPrepackRows}`);
  console.log(`[Dish nutrition] calculated=${summary.fullyCalculatedCount}; partial=${summary.partialCount}; review=${summary.reviewWithoutKnownCount}`);
}

main().catch((error) => {
  console.error(`[Dish nutrition] FAILED: ${error.stack || error.message}`);
  process.exit(1);
});
