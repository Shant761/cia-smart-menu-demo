...
const posterPhotoUrl = (path) => {
  if (!path) return null;
  const text = String(path).trim();
  if (!text) return null;
  if (/^https?:\/\//i.test(text)) return text;
  return `https://joinposter.com${text.startsWith('/') ? '' : '/'}${text}`;
};
...
async function posterRequest(method, params = {}) {
  ...
  if (!response.ok) {
    throw new Error(`Poster ${method} returned HTTP ${response.status}`);
  }
  ...
  try {
    ...
    throw new Error(`Poster ${method} returned invalid JSON`);
  }
...
function normalizeCategory(raw, fallbackIndex = 0) {
  ...
  return {
    id,
    name: String(name),
    order: toNumber(raw?.sort_order ?? raw?.order, fallbackIndex + 1),
    active: String(raw?.hidden ?? '0') !== '1'
  };
}
...
function deriveCategoriesFromProducts(products) {
  ...
  products.forEach((product, index) => {
    ...
    if (category && !seen.has(category.id)) seen.set(category.id, category);
  });
  return [...seen.values()];
}
...
async function loadProductDetails(products) {
  ...
  const details = await mapWithConcurrency(products, 4, async (product) => {
    ...
    try {
      ...
      console.warn(`[Poster sync] Recipe ${productId} skipped: ${error.message}`);
      return null;
    }
  });
  ...
  details.forEach((detail) => {
    const id = detail?.product_id ?? detail?.id;
    if (id != null) map.set(String(id), detail);
  });
  return map;
}
...
async function commitWrites(writes) {
  ...
  for (let i = 0; i < writes.length; i += chunkSize) {
    ...
    writes.slice(i, i + chunkSize).forEach((write) => {
...
async function sync() {
  ...
  if (categoryResult.status === 'fulfilled' && Array.isArray(categoryResult.value)) {
    ...
    console.warn('[Poster sync] menu.getCategories unavailable; categories will be derived from products.');
  }
  ...
  const [restaurantSnapshot, existingProductSnapshot, existingCategorySnapshot] = await Promise.all([
    restaurantRef.get(),
    restaurantRef.collection('products').get(),
    restaurantRef.collection('categories').get()
  ]);

  const existingRestaurant = restaurantSnapshot.exists ? restaurantSnapshot.data() : {};
  const existingProducts = new Map(existingProductSnapshot.docs.map((doc) => [doc.id, doc.data()]));
  const existingCategories = new Map(existingCategorySnapshot.docs.map((doc) => [doc.id, doc.data()]));
  const writes = [];
  ...
  writes.push({
    ...
    data: {
      id: restaurantId,
      type: 'restaurant',
      name: existingRestaurant.name || localized(restaurantName),
  ...
  const allCategoryRef = restaurantRef.collection('categories').doc('all');
  const existingAllCategory = existingCategories.get('all');
  writes.push({
    type: 'set',
    ref: allCategoryRef,
    data: {
      id: 'all',
      name: existingAllCategory?.name || { ru: 'Все', en: 'All', hy: 'Բոլորը' },
      order: 0,
      active: true,
      source: 'cia'
    },
  ...
  posterCategories.forEach((category, index) => {
    ...
    writes.push({
      type: 'set',
      ref: restaurantRef.collection('categories').doc(category.id),
      data: {
        id: category.id,
        name: existing.name || localized(category.name),
        order: category.order || index + 1,
        active: category.active !== false,
        source: 'poster',
        posterCategoryId: category.id,
        updatedAt: now
      },
      options: { merge: true }
    });
  ...
  const currentPosterIds = new Set();
  let activeCount = 0;
  ...
  posterProducts.forEach((product, index) => {
    ...
    const active = !hidden && visibleAtSpot;
    if (active) activeCount += 1;

    const rawPrice = spot?.price ?? mergedPoster?.price ?? product?.price ?? 0;
    const price = toNumber(rawPrice) / 100;
    const photoPath = mergedPoster?.photo_origin || mergedPoster?.photo || product?.photo_origin || product?.photo || null;
    const recipeIngredients = Array.isArray(detail?.ingredients) ? detail.ingredients : [];
    const ingredientNames = recipeIngredients
      .map((ingredient) => String(ingredient?.ingredient_name || '').trim())
      .filter(Boolean);
    const productionDescription = String(detail?.product_production_description || '').trim();

    writes.push({
      type: 'set',
      ref: restaurantRef.collection('products').doc(docId),
      data: {
        id: toNumber(rawId, rawId),
        posterProductId: toNumber(rawId, rawId),
        source: 'poster',
        active,
        category: categoryId,
        name: localized(mergedPoster?.product_name || product?.product_name || `Product ${docId}`),
        description: existing.description || localized(productionDescription),
        ingredients: existing.ingredients || (ingredientNames.length ? { ru: ingredientNames, en: ingredientNames, hy: ingredientNames } : { ru: [], en: [], hy: [] }),
        allergens: existing.allergens || [],
        emoji: existing.emoji || '🍽️',
        image: posterPhotoUrl(photoPath),
        price,
        sortOrder: toNumber(mergedPoster?.sort_order ?? product?.sort_order, index),
        ...
        posterRecipeIngredients: recipeIngredients.map((ingredient) => ({
          ingredientId: ingredient?.ingredient_id ?? null,
          name: ingredient?.ingredient_name ?? '',
          unit: ingredient?.structure_unit ?? ingredient?.ingredient_unit ?? '',
          brutto: toNumber(ingredient?.structure_brutto),
          netto: toNumber(ingredient?.structure_netto),
          locked: String(ingredient?.structure_lock ?? '0') === '1'
        })),
        posterLastSyncAt: now,
        updatedAt: now
      },
  ...
  existingProductSnapshot.docs.forEach((doc) => {
    const data = doc.data();
    if (data?.source === 'poster' && !currentPosterIds.has(doc.id) && data.active !== false) {
      writes.push({
        type: 'update',
        ref: doc.ref,
        data: { active: false, posterLastSyncAt: now, updatedAt: now }
      });
    }
  });
...
sync().catch((error) => {
  ...
  console.error(`[Poster sync] FAILED: ${safeMessage}`);
  process.exit(1);
});
