...
function ingredientDocId(ingredientId, name) {
  if (ingredientId !== null && ingredientId !== undefined && String(ingredientId).trim()) {
    return `poster_${String(ingredientId).trim()}`;
  }
...
async function buildCatalog() {
  ...
  const existingCatalog = new Map(
    existingCatalogSnapshot.docs.map((doc) => [doc.id, doc.data()])
  );
  ...
  for (const productDoc of productSnapshot.docs) {
    ...
    const ingredients = Array.isArray(product.posterRecipeIngredients)
      ? product.posterRecipeIngredients
      : [];
    ...
    for (const ingredient of ingredients) {
      ...
      if (entry.samples.length < 12) {
        entry.samples.push({
          productId: String(product.posterProductId ?? productDoc.id),
          productName,
          netto,
          brutto,
          unit
  ...
  for (const entry of catalog.values()) {
    ...
    let analysisStatus = existing.analysisStatus || 'pending_ai';

    if (!existing.sourceHash) {
      analysisStatus = 'pending_ai';
      pendingAi += 1;
    } else if (sourceChanged) {
      analysisStatus = existing.restaurantVerified === true ? 'needs_review' : 'pending_ai';
  ...
  writes.push({
    ...
    data: {
      ingredientCatalog: {
        uniqueIngredients: catalog.size,
        activePosterProducts,
        recipeRows,
        pendingAi,
        needsReview,
        unchanged,
  ...
  console.log(`[Ingredient catalog] Pending AI: ${pendingAi}, needs review: ${needsReview}, unchanged: ${unchanged}`);
}
