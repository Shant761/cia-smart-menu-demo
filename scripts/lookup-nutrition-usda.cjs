...
function nutrient(food, ids) {
  const n = (food.foodNutrients || []).find(x => ids.includes(Number(x.nutrientId)));
  return Number.isFinite(Number(n?.value)) ? Number(n.value) : null;
}
...
const limit = intEnv('NUTRITION_LIMIT', 100, 1, 500);
