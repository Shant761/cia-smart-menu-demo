...
a(async function main() {
  const snapshot = await db.collection('restaurants').doc(restaurantId).collection('nutrition_review').get();
  if (snapshot.empty) throw new Error(`No nutrition_review entries found for ${restaurantId}. Run Stage 2 first.`);
  ...
  fs.writeFileSync(outputPath, JSON.stringify({ version: '1.0.0', restaurantId, entries }, null, 2) + '\n');
...
function a(fn) { return fn().catch((error) => { console.error(`[CIA nutrition stage 2B] FAILED: ${error?.message || error}`); process.exit(1); }); }
